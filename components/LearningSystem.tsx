        ...
        <div className="flex flex-col lg:flex-row gap-16 items-center">
          <div className="w-full lg:w-1/2">
            ...
            <div className="mt-10 space-y-8">
              {[
                { step: 1, title: 'Daily Test & Review', desc: '매 수업 시작 시 단어/리뷰 테스트 진행 (통과 못할 시 재시험)' },
                { step: 2, title: '1:1 Coaching Clinic', desc: '수업 직후 담당 강사와 1:1로 오답 체크 및 질의응답' },
                { step: 3, title: 'Weekly Report', desc: '주간 학습 태도, 과제 수행도, 시험 점수를 카카오톡 발송' }
              ].map((item, i) => (
                <div key={i} className="flex items-start gap-6">
                  <div className="h-12 w-12 flex-shrink-0 flex items-center justify-center rounded-2xl bg-white text-primary font-bold text-xl shadow-md border border-slate-100 dark:bg-slate-800 dark:border-slate-700">{item.step}</div>
                  <div>
            ...
            <div className="rounded-3xl bg-white p-8 shadow-2xl dark:bg-slate-800 border border-slate-100 dark:border-slate-700 transform md:rotate-1 hover:rotate-0 transition-transform duration-500">
              <div className="flex items-center justify-between border-b border-slate-100 pb-6 dark:border-slate-700">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-bold text-lg text-slate-900 dark:text-white">김철수 학생 학습 리포트</p>
                    <p className="text-sm text-slate-500">2023년 10월 3주차</p>
                  </div>
                </div>
                <span className="rounded-full bg-green-100 px-4 py-1.5 text-xs font-bold text-green-700 tracking-wide">양호</span>
              </div>
              <div className="mt-8 space-y-6">
                {[
                  { label: '어휘 (Vocabulary)', score: '96/100', width: '96%', animate: true },
              ...
              <div className="mt-8 rounded-xl bg-blue-50/80 p-5 text-sm text-blue-900 dark:bg-blue-900/30 dark:text-blue-200 leading-relaxed border border-blue-100 dark:border-blue-800">
                <strong>👨‍🏫 선생님 코멘트:</strong> 철수는 이번 주 단어 암기가 매우 우수합니다. 관계대명사 파트에서 실수가 있었으나 1:1 클리닉을 통해 보완했습니다. 다음 주엔 독해 속도에 집중할 예정입니다.
              </div>
            </div>
          </div>
        </div>
