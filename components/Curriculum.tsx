    ...
    <section className="py-20 bg-white dark:bg-slate-900" id="curriculum">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-3xl font-black text-slate-900 dark:text-white sm:text-4xl">학년별 최적화 커리큘럼</h2>
          <p className="mt-4 text-lg text-slate-600 dark:text-slate-300">기초부터 실전까지, 빈틈없는 단계별 학습</p>
        </div>
          ...
          {courses.map((course, i) => (
            ...
            >
                ...
                {course.points.map((p, idx) => (
                  <div key={idx} className="flex items-start gap-4">
                    ...
                    <div className="text-[15px] text-slate-700 dark:text-slate-300 leading-relaxed">
                      <strong className="block mb-1 text-slate-900 dark:text-white">{p.label}</strong>
                      <span className="text-slate-500 dark:text-slate-400 text-sm">{p.desc}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="p-6 border-t border-slate-50 dark:border-slate-700 bg-slate-50/30 dark:bg-slate-900/30">
                <a className="flex items-center justify-center gap-2 w-full text-center py-3.5 rounded-2xl text-sm font-bold text-slate-600 bg-white border border-slate-200 hover:text-primary hover:border-primary/30 transition-all duration-300 shadow-sm hover:shadow-md dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300" href="#consultation">
                  상세 커리큘럼 문의 
