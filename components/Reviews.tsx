...
const Reviews: React.FC = () => {
  ...
  const moreReviews = [
    {
      id: 1,
      content: "이곳만의 커리큘럼도 좋은 점이지만 아이들 잘 파악하셔서 관리잘해주시고 학습습관까지 잘 배울 수 있는곳이기에 만족합니다. 다른데 거쳐오면서 영어시작할 때부터 다녔으면 더욱 좋았겠다라는 아쉬움도 있지만 제대로 공부 할 나이에 늦지않게 알게되어 안도하는 면도 있네요~",
      author: "중등 학부모님",
      date: "2023.11.05"
    },
    {
      id: 2,
      content: "초등아이 늦게 영어를 시작한 편이라 걱정이 많았는데 원서리딩에 감동♡ 매일매일 녹음숙제에, 단어시험부터 문장까지 꼼꼼하고 탄탄히 가르쳐주십니다. 습관을 잘 만들어주셔서 이제는 아이가 스스로 녹음을 합니다^^",
      author: "초6 학부모님",
      date: "2023.12.12"
    },
    {
      id: 3,
      content: "고등 중등 두아이 달콤플러스영어 다니고있어요 단어시험부터 관리해주고 원장님 직강이 있어 학습태도나 학생지도 점검해주셔서 늘감사합니다~~ 이사와서 다니는데 정말 감사합니다 🙏",
      author: "고등/중등 학부모님",
      date: "2024.01.10"
    }
  ];
  ...
  return (
    <section className="py-24 bg-white dark:bg-slate-900" id="reviews">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {/* More Reviews (Collapsible) */}
        {showMore && (
          <div className="mt-8 grid gap-6 md:grid-cols-3 max-w-7xl mx-auto animate-fade-in-up">
            {moreReviews.map((review) => (
              <div key={review.id} className="bg-slate-50 rounded-2xl p-6 border border-slate-100 shadow-sm hover:shadow-md transition-all dark:bg-slate-800 dark:border-slate-700">
                ...
                <div className="flex items-center gap-3 border-t border-slate-200 pt-4 dark:border-slate-700">
                  ...
                  <div>
                    <p className="font-bold text-xs text-slate-900 dark:text-white">{review.author}</p>
        ...
        <div className="mt-12 text-center">
          ...
          >
            ...
            <span className={`material-symbols-outlined text-lg transition-transform duration-300 ${showMore ? 'rotate-45' : ''}`}>add</span>
          </button>
        </div>
      </div>
    </section>
