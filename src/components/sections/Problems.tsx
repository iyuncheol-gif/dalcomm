    ...
    <section className="py-20 bg-white dark:bg-slate-900" id="problems">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-12 text-center">
          <span className="inline-block px-3 py-1 rounded-full bg-blue-50 text-primary font-bold tracking-wider text-xs uppercase mb-3 dark:bg-blue-900/20">
            Problem & Solution
          </span>
          <h2 className="text-3xl font-black text-slate-900 dark:text-white sm:text-4xl word-keep-all">
            학부모님들의 가장 큰 고민,
            <br className="block sm:hidden" /> 무엇인가요?
          </h2>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {problems.map((p, i) => (
            <div
              key={i}
              className="flex flex-col rounded-3xl overflow-hidden border border-slate-200 shadow-lg bg-white dark:bg-slate-800 dark:border-slate-700 group hover:-translate-y-1 transition-transform duration-300"
            >
              <div className="p-8 bg-slate-50 dark:bg-slate-800/50 flex-1 relative">
                ...
                <h3 className="mb-3 text-xl font-bold text-slate-900 dark:text-white word-keep-all">
                  {p.title}
                </h3>
                <p className="text-slate-600 dark:text-slate-300 leading-relaxed word-keep-all text-sm mb-4">
                  {p.desc}
                </p>
                <div className="mt-4 pt-4 border-t border-slate-200/60 dark:border-slate-700">
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-primary font-bold text-sm leading-snug word-keep-all whitespace-pre-line">
                      {p.solution}
                    </p>
                  </div>
                </div>
            ...
            {[
              ...
              <div key={i} className="flex flex-col items-center gap-4 group/item">
                <div className="w-16 h-16 rounded-2xl bg-white/10 backdrop-blur-md flex items-center justify-center shadow-inner border border-white/20 group-hover/item:bg-accent group-hover/item:scale-110 transition-all duration-300">
                  <span className="material-symbols-outlined text-white text-3xl group-hover/item:text-primary transition-colors">
                    {feat.icon}
                  </span>
                </div>
                <span className="font-bold text-white text-base tracking-tight lg:text-lg">
                  {feat.label}
                </span>
              </div>
            ))}
